...
import { userLogin, staffLogin } from '../../redux/actions';
...
class LandingPage extends Component {
	constructor(props) {
		...
		this.onLoginStaff = this.onLoginStaff.bind(this);
	...
	*/
	...
	onLoginStaff(e) {
		e.preventDefault();
		const { triggerLoginStaff } = this.props;
        triggerLoginStaff(this.userUname.value, this.userPass.value);
	}
	 ...
	 */
	responseHandler() {
		const { login: { data: { code, message, data, accessToken } } } = this.props;
		toast.dismiss();
		if (code === 100) {
				...
				<Row>
					...
					<Col md={4} sm={6} xs={8}>
						<p className='text-center'>
							<h2 className='logo-text'>Wellness Admin</h2>
						</p>
						<section className='user-login' id='user-login'>
							<Form>
								...
								<p className='text-center'>
									<button className="litnite-btn " onClick={this.onLogin}>LOGIN AS ADMIN&nbsp;&nbsp;&nbsp;<FontAwesome name="chevron-right"/></button>&nbsp;
									<button className="litnite-btn" onClick={this.onLoginStaff}>LOGIN AS STAFF&nbsp;&nbsp;&nbsp;<FontAwesome name="chevron-right"/></button>

								</p>
							</Form>
							<br/><br/><br/>
							<hr className='line-break'/>
							<p className='text-center'>Wellness-Admin&copy;2019</p>
						</section>
					</Col>
					<Col md={4} sm={3} xs={2}></Col>
				</Row>
...
const mapDispatchToProps = dispatch => {
	return {
		...
		triggerLoginStaff: (email, password) => dispatch(staffLogin({email, password}))
	};
}
